...
def download_metadata_zips(num_files_download, outdir, overwrite=False, datetime="20200401"):
    '''
    download enwiki data dumps, number defines how many
    '''

    url = 'https://dumps.wikimedia.org/enwiki/'+ datetime + '/'
...
#         print(page_title + " " + str(len(revisions)))
        ...
        if len(revisions) < min_revisions or (len(articles) != 0 and page_title not in articles):
            elem.clear()
...
#                     version += 1
        ...
        if len(page_results) > min_revisions and (len(articles) == 0 or page_title in articles):
            ...
            print(str(curr_count) + ": Writing {} {} revisions to {}".format(page_title, len(page_results), outfile))
